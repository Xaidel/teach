 ...
 *   reference-solution length, with a small absolute floor, capped at the
 *   solution's own token count so a full verbatim copy always blocks (issue
 *   #64), applied two-tier: near-zero tolerance at Levels 0-3, a much higher
 *   tolerance (~45% starting point) at Level 4. Level 5 is unchecked — the
 *   learner has explicitly opted into the full solution.
 ...
 * requests, general paraphrase/restructuring, and unshielded `explainConcept`
 * output (no exercise context in v1; issue #65).
 */
...
export function shieldThresholdTokens(
  ...
  return Math.min(
    Math.max(PROMPT_SHIELD_TOKEN_FLOOR, ratioTokens),
    solutionTokenCount,
  )
}
