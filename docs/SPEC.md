...
> **Scope note (issue #65):** this leakage guarantee covers **hints only** in v1. `explainConcept` output is not shielded — explanations are standalone reference material with no exercise/reference-solution context to compare against (see "Prompt Shield" and "Prompt Shield near-match algorithm" in Implementation Decisions).
...
- **Per-language allowed dependency set — mechanism**: each language's warm-cache skeleton project *is* the allowed-dependency list — `sandbox/<lang>/skeleton/{Cargo.toml,go.mod,requirements.txt}`, committed alongside that language's Dockerfile, one artifact serving both roles so "allowed" and "pre-compiled" can't drift apart. Every dependency is pinned to an exact version, never a range. Curated by direct edit (no review/approval step); `sandbox:build`'s rebuild trigger (ADR-0013) broadens from "Dockerfile changes" to "anything under `sandbox/<lang>/` changes." The actual per-language dependency list content stays out of scope, deferred to build tickets #8/#19/#20. (ADR-0018, refines ADR-0011, amends ADR-0013; resolved as wayfinder ticket [#33](https://github.com/Xaidel/teach/issues/33) on the [AI Learning Platform v1 map](https://github.com/Xaidel/teach/issues/21).)
...
- **Prompt Shield**: Solution-leakage detection is a deterministic check — compare AI Teacher output against the Pre-Flight-verified reference solution, gated by current hint level — not a second LLM call. **Covers hints only in v1**: `explainConcept` output is not shielded (no exercise/reference-solution context to compare against; accepted gap, issue #65 — see below). Prompt-injection detection (the other half of Section 5's guardrail) is left open; add an LLM-based check later only if the deterministic layer proves insufficient. (ADR-0008)
- **Prompt Shield near-match algorithm**: Fragment-level comparison — scans for any fragment of the Teacher's response that closely matches any fragment of the reference solution, not a whole-message score — checked per-hint (not cumulative across an attempt). Both texts are normalized (whitespace collapsed, comments stripped) then split by one minimal, language-agnostic lexer (token boundaries only, no per-language keyword awareness); closeness is token overlap, not edit distance or AST comparison. The block threshold is a percentage of that exercise's own solution length (with a small absolute floor, capped at the solution's own length so a verbatim copy always blocks — issue #64), applied two-tier: near-zero tolerance at Levels 0–3 (no real code expected), a much higher tolerance (~40–50% starting point) at Level 4 (partial solution expected by design); Level 5 is unchecked. Explicitly accepted v1 gaps: renamed-variable leaks (identifier normalization not implemented), cumulative leakage across repeated same-level hint requests, general paraphrase/restructuring (already named in ADR-0008), and unshielded `explainConcept` output (no exercise context; issue #65). (ADR-0012; resolved as wayfinder ticket [#25](https://github.com/Xaidel/teach/issues/25) on the [AI Learning Platform v1 map](https://github.com/Xaidel/teach/issues/21).)
...
- **Generated test source storage**: `exercises` gets a nullable `test_source` **text** column, sibling to `reference_solution` — the full generated test file's source as one blob (PRD §13's `evaluation.tests` names, e.g. `ownership_test`/`borrowing_test`, read as multiple test functions inside that one file, not multiple stored files). `generateExercise` populates it for `implement`/`debug`-mode rows alongside `reference_solution` and `evaluation_rubric`; it stays `NULL` for `explain`-mode rows, same rule as `evaluation_rubric`. This is the storage location for what ADR-0011 already calls "the exercise's test harness" — Stage 1 writes this column's value into the Sandbox Workspace on every attempt against the exercise, not just once at generation/Pre-Flight time; the per-language file path it's written to (Rust `tests/*.rs`, Go `_test.go`, Python `test_*.py`) stays sandbox-orchestration mechanics (ADR-0011), applied per-language by build tickets #8/#19/#20. (ADR-0019, amending ADR-0010; resolved as wayfinder ticket [#34](https://github.com/Xaidel/teach/issues/34) on the [AI Learning Platform v1 map](https://github.com/Xaidel/teach/issues/21).)
