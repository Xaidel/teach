...
- **Threshold units: percentage of that exercise's own reference-solution length**, not an absolute constant — so a 3-line solution and a 40-line solution are held to proportionally fair standards. A small absolute floor (on the order of a handful of tokens) prevents a single shared keyword or short common phrase from ever triggering a block by itself. The threshold is additionally capped at the solution's own token count, so a full verbatim copy always blocks even when the solution is shorter than the floor — the floor alone would otherwise let a complete sub-floor copy pass at the near-zero-tolerance levels (issue #64).
