...
- `draftConceptGraph()`'s existing contract (ADR-0011/ticket #23) was specified as one call producing a language's graph; this ADR's runtime-gap case additionally needs it (or a variant) to draft a single concept scoped against the existing graph. The exact function signature for that is left to whoever implements ticket #7/#19/#20 or the Class B extraction path — not fixed here.
  - **Resolved by ticket #13** (Class B: Tactical Flow): `draftConceptGraph()`'s input gained an optional `focusConcept: { slug, description }`. When set, the prompt switches to a single-concept mode — the model returns exactly the given slug (never renamed) with a difficulty informed by the description, and no edges — rather than a broad draft. The Class B extraction path (`tactical-sprint` feature) calls this through `concepts.server.ts`'s `draftFocusedConcept()`, which persists the result through the same insert/validation-report logic `draftConcepts()` (the broad path) already used, and rejects a draft whose output omits the requested slug.
